...
AI CLUSTERING (VPE-001B+)
  │  Inter-model ARI = 0.812 · ELUZ/SHA/NAR recovered · Axes NOT recovered
  ▼
OPEN: VPE-001A
...
**Key finding:** Inter-model cluster ARI = 0.812 indicates strong convergence on a common structure. Mean ARI vs designed axes = 0.150 indicates this structure is not the designed axes. **LUXVAR's structure, as consistently recovered by independent language models, is morphemic, not axial.**
...
4. Recoverable morpheme families (ELUZ, SHA, NAR) — inter-AI ARI = 0.812
...
## Post-Publication Addendum (July 2026)

**Note added after initial publication.**

**1. Numerical correction.** The inter-model ARI value (Claude↔Gemini) is reported throughout this document as 0.812, consistent with the value reported across the VARZIN project's other public materials. An earlier internal draft had used 0.718 for this same comparison; this appears to have been a superseded intermediate calculation. The author was unable to independently re-verify the raw pairwise scoring at the time of this correction and has standardized on 0.812 as the value consistent with the rest of the documented record. Readers requiring the underlying raw model responses for independent re-verification should contact the author.

**2. Independent computational audit.** Following publication of this preprint, a separate four-stage adversarial audit examined whether VARZIN's structure-detection computational pipeline (the "benchmark_003" lineage, distinct from but methodologically related to the VPE-001B+ AI-clustering analysis reported here) offers a distinct, generalizable computational contribution beyond standard specialized baselines. The audit's finding — documented in full in VARZIN-COMPUTATIONAL-AUDIT-CLOSURE-v1 — was that no such distinct contribution could be established: the one measurable internal advantage of the pipeline did not survive testing on an independently-coded data generator, and could not be attributed to any specific feature.

This audit does not directly re-test the VPE-001B+ AI-clustering result reported in Section 4.5 (a different analysis: three language models clustering a fixed 30-word list, rather than a trained classifier evaluated on synthetic benchmarks). However, given the methodological relationship between the two lines of work, readers should treat the "AI-Recoverable" claim in this paper's title with the same caution the audit recommends for the broader pipeline, pending independent replication of the VPE-001B+ clustering result specifically. The paper's non-AI-dependent findings (phonotactic uniqueness, generator balance, negative semantic-axis results) are unaffected by this audit, as they do not rely on the audited pipeline.

The single remaining non-circular empirical test proposed in Section 6 (VPE-001A, human rater study) has not yet been run and remains the most direct way to independently assess whether LUXVAR's morphemic structure is perceptible to naive observers, independent of any AI system.

---

## Second Post-Publication Addendum (August 2026) — "AI-Recoverable" Claim REJECTED

**Note added following independent replication.**

The caution flagged in the July 2026 addendum above has since been resolved directly rather than left pending. A dedicated adversarial audit, reported in *"Morphemes vs. Manifolds: Diagnosing Structural Blindness and Morphological Hijacking in Large Language Models via Finite Affine Orbits"* (Zenodo, DOI: 10.5281/zenodo.22101179; companion Level-1 monograph DOI: 10.5281/zenodo.22036769), directly tested whether language models recover the *true designed algebraic axes* — rather than measuring agreement between models on their own groupings, as Section 4.5 of this paper does.

Under a strict "Semantic Starvation" protocol, with the classification target's ground-truth orbit labels (not each other's outputs) as the scoring reference, and using Hungarian-alignment ARI across 30 independent seeds per model:

- **Mistral 7B Instruct:** ARI = −0.0337 ± 0.039 (random-chance baseline: −0.0583 ± 0.045)
- **Llama 3 8B:** ARI = 0.0152 ± 0.028

Both models scored statistically indistinguishable from random chance against the true axes, and their confusion matrices show systematic grouping by shared surface spelling (e.g. the shared root ELŪZ) rather than by the underlying Aff(Z₁₂) orbit structure — a failure mode termed **"Morphological Hijacking."**
...
DOI archive status (following independent audit): varzin.org/all-dois.html  
ORCID: 0009-0000-4690-6842
...
*Preprint · not peer-reviewed · v2.1 (with July 2026 and August 2026 addenda) · June 2026*  
